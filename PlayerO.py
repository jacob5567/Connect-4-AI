...
def next_move(board, player, lookup_table):
    col = alphabeta(board, ALPHA_BETA_DEPTH_LEVEL, -
                    math.inf, math.inf, True, player)[1]
    return col
    ...
    if depth == 0 or board.winner():
        ...
        if maximizingPlayer:
            ...
            for col in board.available_moves():
                ...
                score = alphabeta(boardCopy, depth - 1,
                                  alpha, beta, False, player)[0]
                if score > value:
                    value = score
                    bestColumn = col
...
def scoreBoard(game, player):
    ...
    score += [r[3]
              for r in gameBoard].count(player) * PREFER_CENTRAL_COLUMN_MULTIPLIER

    # prefer closer to center columns
    score += [r[2]
              for r in gameBoard].count(player) * PREFER_NEARBY_COLUMNS_MULTIPLIER
    score += [r[4]
              for r in gameBoard].count(player) * PREFER_NEARBY_COLUMNS_MULTIPLIER
    ...
    for row in range(6):
        ...
        for col in range(4):
            ...
            # weights higher sets better
            score += scoreSelection(selection, player) * \
                (row / PREFER_HIGHER_ROWS_MULTIPLIER)
    # vertical check
    for col in range(7):
